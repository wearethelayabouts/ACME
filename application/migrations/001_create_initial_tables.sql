...
CREATE TABLE `categories` (
  ...
  `isHub` tinyint(1) NOT NULL DEFAULT '0',
  `rating` tinytext NOT NULL,
  `rating_description` tinytext NOT NULL,
  `returnAllContent` int(1) NOT NULL DEFAULT '0',
  `allowZIP` int(1) NOT NULL DEFAULT '0',
  `oldestFirst` int(1) NOT NULL DEFAULT '0',
  `allowPlayAll` int(1) NOT NULL,
  `navComic` int(1) NOT NULL,
  `comicnav_first` int(11) NOT NULL,
  `comicnav_back` int(11) NOT NULL,
  `comicnav_next` int(11) NOT NULL,
  `comicnav_last` int(11) NOT NULL,
  `listPriority` int(11) NOT NULL DEFAULT '0' COMMENT 'list priority level. higher numbers come first',
  `desc` text NOT NULL,
...
CREATE TABLE `content` (
  ...
  `featured_image` int(10) unsigned DEFAULT NULL,
  `featured_name` tinytext,
  `customEmbed` text NOT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE `contentauthors` (
  ...
  `role` int(10) unsigned NOT NULL COMMENT 'the role that the user had in the project, matched up with one of the roles inside `contentroles`',
  `showIcon` int(1) NOT NULL DEFAULT '1',
  PRIMARY KEY (`id`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1;

CREATE TABLE `contentroles` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT 'internal role identifier',
  `title` mediumtext NOT NULL COMMENT 'name of the role',
  PRIMARY KEY (`id`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1;
...
CREATE TABLE `files` (
  ...
  `internalDescription` tinytext NOT NULL COMMENT 'Put in a description of the file to help keep track of what file this is.',
  PRIMARY KEY (`id`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1;

CREATE TABLE `links` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT 'internal link identifier',
  `link_type` smallint(5) unsigned NOT NULL COMMENT 'object type; *0=other; 1=category; 2=content; 3=comment',
  `objectid` int(10) unsigned NOT NULL COMMENT 'used only if type!=0 (points to column "id" in table "categories" if type=1; "content" if type=2; or "comments" if type=3)',
  `url` tinytext NOT NULL COMMENT 'used only if type=0; an external or otherwise not_system_generated link',
  `name` tinytext NOT NULL COMMENT 'actual name that we would see on the links bar',
  `rolloverimage` int(11) unsigned NOT NULL,
